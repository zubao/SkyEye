...
class SkyEyeAnalyticsClassVisitor extends ClassVisitor {
    ...
    private SkyEyeAnalyticsTransformHelper transformHelper

    private ClassNameAnalytics classNameAnalytics

    private ArrayList<SkyEyeAnalyticsMethodCell> methodCells = new ArrayList<>()

    private int version

    private HashMap<String, SkyEyeAnalyticsMethodCell> mLambdaMethodCells = new HashMap<>()

    private final String ANNOTOTION_CLASS_DESC  = "Lcom/pingan/pad/skyeye/data/SkyEyeDataInstrumented;"

    private final String TRACK_VIEW_ON_CLICK        = "Lcom/pingan/pad/skyeye/data/SkyEyeDataTrackViewOnClick;"
    private final String TRACK_VIEW_ON_IGNORE       = "Lcom/pingan/pad/skyeye/data/SensorsDataIgnoreTrackOnClick;"
    private final String TRACK_VIEW_ON_EVENT        = "Lcom/pingan/pad/skyeye/data/SensorsDataTrackEvent;"
    ...
    SkyEyeAnalyticsClassVisitor(final ClassVisitor classVisitor, ClassNameAnalytics classNameAnalytics, SkyEyeAnalyticsTransformHelper transformHelper) {
        super(SkyEyeAnalyticsUtil.ASM_VERSION, classVisitor)
    ...
    void visitEnd() {
        ...
        if (SkyEyeAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
            MethodVisitor mv
            // 添加剩下的方法，确保super.onHiddenChanged(hidden);等先被调用
            Iterator<Map.Entry<String, SkyEyeAnalyticsMethodCell>> iterator = SkyEyeAnalyticsHookConfig.FRAGMENT_METHODS.entrySet().iterator()
            while (iterator.hasNext()) {
                Map.Entry<String, SkyEyeAnalyticsMethodCell> entry = iterator.next()
                String key = entry.getKey()
                SkyEyeAnalyticsMethodCell methodCell = entry.getValue()
                ...
                visitMethodWithLoadedParams(mv, Opcodes.INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, methodCell.agentName, methodCell.agentDesc, methodCell.paramsStart, methodCell.paramsCount, methodCell.opcodes)
                mv.visitInsn(Opcodes.RETURN)
                mv.visitMaxs(methodCell.paramsCount, methodCell.paramsCount)
                mv.visitEnd()
                mv.visitAnnotation(ANNOTOTION_CLASS_DESC, false)
            }
        }
    ...
    FieldVisitor visitField(int access, String name, String descriptor, String signature, Object value) {
        if (classNameAnalytics.isSensorsDataAPI) {
            if ('VERSION' == name) {
                ...
                if (SkyEyeAnalyticsTransform.MIN_SDK_VERSION > version) {
                    String errMessage = "你目前集成的神策埋点 SDK 版本号为 v${version}，请升级到 v${SkyEyeAnalyticsTransform.MIN_SDK_VERSION} 及以上的版本。详情请参考：https://github.com/sensorsdata/sa-sdk-android"
                ...
                if (minPluginVersion != "" && minPluginVersion != null) {
                    if (SkyEyeAnalyticsTransform.VERSION < minPluginVersion) {
                        String errMessage = "你目前集成的神策插件版本号为 v${SkyEyeAnalyticsTransform.VERSION}，请升级到 v${minPluginVersion} 及以上的版本。详情请参考：https://github.com/sensorsdata/sa-sdk-android-plugin2"
                        Logger.error(errMessage)
                        throw new Error(errMessage)
                    }
                }
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        SkyEyeAnalyticsDefaultMethodVisitor sensorsAnalyticsDefaultMethodVisitor = new SkyEyeAnalyticsDefaultMethodVisitor(methodVisitor, access, name, desc) {
            ...
            void visitEnd() {
                ...
                if (isHasTracked) {
                    ...
                    visitAnnotation(ANNOTOTION_CLASS_DESC, false)
            ...
            void visitInvokeDynamicInsn(String name1, String desc1, Handle bsm, Object... bsmArgs) {
                ...
                try {
                    ...
                    SkyEyeAnalyticsMethodCell sensorsAnalyticsMethodCell = SkyEyeAnalyticsHookConfig.LAMBDA_METHODS.get(Type.getReturnType(desc1).getDescriptor() + name1 + desc2)
            ...
            protected void onMethodEnter() {
                super.onMethodEnter()
                nameDesc = name + desc
                pubAndNoStaticAccess = SkyEyeAnalyticsUtil.isPublic(access) && !SkyEyeAnalyticsUtil.isStatic(access)
            ...
            void handleCode(){
                 ...
                 */
                if (SkyEyeAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
                    SkyEyeAnalyticsMethodCell sensorsAnalyticsMethodCell = SkyEyeAnalyticsHookConfig.FRAGMENT_METHODS.get(nameDesc)
                    if (sensorsAnalyticsMethodCell != null) {
                        visitedFragMethods.add(nameDesc)
                        if (isSetUserVisibleHint) {
                            methodVisitor.visitVarInsn(ALOAD, 0)
                            methodVisitor.visitVarInsn(ILOAD, variableID)
                            methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, sensorsAnalyticsMethodCell.agentName, sensorsAnalyticsMethodCell.agentDesc, false)
                            ...
                            visitMethodWithLoadedParams(methodVisitor, Opcodes.INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, sensorsAnalyticsMethodCell.agentName, sensorsAnalyticsMethodCell.agentDesc, sensorsAnalyticsMethodCell.paramsStart, sensorsAnalyticsMethodCell.paramsCount, sensorsAnalyticsMethodCell.opcodes)
                        }
                        isHasTracked = true
                        return
                    }
                 ...
                 */
                if (transformHelper.extension.lambdaEnabled) {
                    SkyEyeAnalyticsMethodCell lambdaMethodCell = mLambdaMethodCells.get(nameDesc)
                    if (lambdaMethodCell != null) {
                        ...
                        boolean isStaticMethod = SkyEyeAnalyticsUtil.isStatic(access)
                        if (!isStaticMethod) {
                            if (lambdaMethodCell.desc == '(Landroid/view/MenuItem;)Z') {
                                ...
                                methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, lambdaMethodCell.agentName, '(Ljava/lang/Object;Landroid/view/MenuItem;)V', false)
                                isHasTracked = true
                                return
                            }
                        }
                        ...
                        methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, lambdaMethodCell.agentName, lambdaMethodCell.agentDesc, false)
                        isHasTracked = true
                        return
                    }
                }
                ...
                if (nameDesc == 'setJSResponder(IIZ)V' && mClassName == 'com/facebook/react/uimanager/NativeViewHierarchyManager') {
                    ...
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, "trackRN", "(Ljava/lang/Object;IIZ)V", false)
                    isHasTracked = true
                    return
                }
                 ...
                 */
                if (SkyEyeAnalyticsUtil.isTargetMenuMethodDesc(nameDesc)) {
                    methodVisitor.visitVarInsn(ALOAD, 0)
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, "trackMenuItem", "(Ljava/lang/Object;Landroid/view/MenuItem;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (nameDesc == 'onDrawerOpened(Landroid/view/View;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, "trackDrawerOpened", "(Landroid/view/View;)V", false)
                    isHasTracked = true
                    return
                } else if (nameDesc == 'onDrawerClosed(Landroid/view/View;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, "trackDrawerClosed", "(Landroid/view/View;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (!SkyEyeAnalyticsUtil.isTargetClassInSpecial(mClassName)) {
                ...
                if (nameDesc == 'onItemSelected(Landroid/widget/AdapterView;Landroid/view/View;IJ)V' || nameDesc == "onListItemClick(Landroid/widget/ListView;Landroid/view/View;IJ)V") {
                    ...
                    methodVisitor.visitVarInsn(ALOAD, 2)
                    methodVisitor.visitVarInsn(ILOAD, 3)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, "trackListView", "(Landroid/widget/AdapterView;Landroid/view/View;I)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (eventName != null && eventName.length() != 0) {
                    ...
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, "track", "(Ljava/lang/String;Ljava/lang/String;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (mInterfaces != null && mInterfaces.length > 0) {
                    if (isOnItemClickMethod && mInterfaces.contains('android/widget/AdapterView$OnItemClickListener')) {
                        ...
                        for (interfaceName in mInterfaces) {
                            SkyEyeAnalyticsMethodCell sensorsAnalyticsMethodCell = SkyEyeAnalyticsHookConfig.INTERFACE_METHODS.get(interfaceName + nameDesc)
                            if (sensorsAnalyticsMethodCell != null) {
                                visitMethodWithLoadedParams(methodVisitor, INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, sensorsAnalyticsMethodCell.agentName, sensorsAnalyticsMethodCell.agentDesc, sensorsAnalyticsMethodCell.paramsStart, sensorsAnalyticsMethodCell.paramsCount, sensorsAnalyticsMethodCell.opcodes)
                                isHasTracked = true
                                return
                            }
                        }
            ...
            void trackViewOnClick(MethodVisitor mv, int index) {
                ...
                mv.visitMethodInsn(INVOKESTATIC, SkyEyeAnalyticsHookConfig.SENSORS_ANALYTICS_API, "trackViewOnClick", "(Landroid/view/View;)V", false)
            }
            ...
            AnnotationVisitor visitAnnotation(String s, boolean b) {
                if (s == TRACK_VIEW_ON_CLICK) {
                    isSensorsDataTrackViewOnClickAnnotation = true
                    Logger.info("发现 ${name}${desc} 有注解 @SkyEyeDataTrackViewOnClick")
                } else if (s == TRACK_VIEW_ON_IGNORE) {
                    isSensorsDataIgnoreTrackOnClick = true
                    Logger.info("发现 ${name}${desc} 有注解 @SkyEyeDataIgnoreTrackOnClick")
                } else if (s == ANNOTOTION_CLASS_DESC) {
                    isHasInstrumented = true
                } else if (s == TRACK_VIEW_ON_EVENT) {
                    return new AnnotationVisitor(SkyEyeAnalyticsUtil.ASM_VERSION) {
                        @Override
        ...
        if (version <= Opcodes.V1_5) {
            return new SkyEyeAnalyticsJSRAdapter(SkyEyeAnalyticsUtil.ASM_VERSION, sensorsAnalyticsDefaultMethodVisitor, access, name, desc, signature, exceptions)
        }
